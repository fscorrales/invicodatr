...
#'
#' @export
rpw_sgf_resumen_rend_obra <- function(path, write_csv = FALSE,
                                      write_sqlite = FALSE,
                                      overwrite_sql = FALSE){

  Ans <- purrr::map_df(path, ~ try_read(read_sgf_resumen_rend_obra(.x)))

  if (write_csv == TRUE) {
    write_csv(Ans, "Resumen Rendiciones por Obra SGF.csv")
  }

  if (write_sqlite == TRUE) {

    sql_db <- "sgf"
    sql_table <- "resumen_rend_obra"
    sql_key_var_1 <- "mes"
    sql_key_var_2 <- "origen"
